...
import org.eclipse.birt.report.engine.emitter.xls.*;
import org.eclipse.birt.report.engine.emitter.xls.BlankData.Type;
import org.eclipse.birt.report.engine.emitter.xls.DataCache.DataFilter;
import org.eclipse.birt.report.engine.emitter.xls.DataCache.RowIndexAdjuster;
...
public class Page
{
	...
	{
		outputStyleInQuote( style.getFontFamily( ), style.getFontWeight( ),
				style.getFontStyle( ), buffer );
	...
	private void outputStyleInQuote( String fontFamily, String bold,
			String italic, StringBuffer buffer )
	{
	...
	private boolean needOutputInQuote( String fontFamily, String bold,
			String italic )
	{
		return fontFamily != null
				|| ( bold != null && "bold".equalsIgnoreCase( bold ) )
	...
	protected Data createData( Object value, StyleEntry style,
			int rowSpanOfDesign )
	{
	...
	public Data createData( Object value, int styleId, int dataType,
			int rowSpanOfDesign )
	{
	...
	public void addEmptyDataToContainer( XlsContainer child,
			XlsContainer parent )
	{
	...
	public void addEmptyDataToContainer( StyleEntry style,
	...
	public void synchronize( float height, XlsContainer rowContainer )
	{
		...
		int startColumnIndex = axis
				.getColumnIndexByCoordinate( startCoordinate );
		int endColumnIndex = axis.getColumnIndexByCoordinate( endCoordinate );

		int maxRowIndex = 0;
		// contains the index of the last row in each column
		// this is used to track how much the columns must be filled using rowspan
		...
		for ( int currentColumnIndex = startColumnIndex; currentColumnIndex < endColumnIndex; currentColumnIndex++ )
		{
			...
			SheetData lastData = currentCache
		...
		for ( int currentColumnIndex = startColumnIndex; currentColumnIndex < endColumnIndex; currentColumnIndex++ )
		{
			int rowspan = maxRowIndex
					- rowIndexes[currentColumnIndex - startColumnIndex];
			SheetData upstair = currentCache
			        .getColumnLastData( currentColumnIndex );
			if ( rowspan > 0 )
			{
				if ( upstair != null
	...
	/**
	 * Expand the cell of the passed sheet data.
	 * Creates empty data entries to cover the given rowspan and updates
	...
	private void spanWithEmptyData( SheetData sheetData, int currentColumnIndex, int rowspan )
	{
		...
		for ( int p = 1; p <= rowspan; p++ )
		{
			...
			if ( styleId != -1 )
			{
				...
				// the bottom border and side borders have been copied through the style copy
	...
	public void calculateRowHeight( SheetData[] rowData, boolean isAuto )
	{
		...
		float lastRowHeight = rowIndex > 0 ? currentCache
		        .getRowHeight( rowIndex - 1 ) : 0;
